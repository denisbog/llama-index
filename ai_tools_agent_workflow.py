...
class SimpleAgentWorkflowMeta(WorkflowMeta, ABCMeta):
    """Metaclass for AgentWorkflow that inherits from WorkflowMeta."""


class SimpleAgentWorkflow(Workflow, PromptMixin, metaclass=SimpleAgentWorkflowMeta):
    ...
    async def setup_agent(self, ctx: Context, ev: AgentInput) -> AgentSetup:
        ...
        llm_input = [
                ChatMessage(role="user", content=self.state_prompt.format(state=state, msg=user_msg_str)),
        ]
        ...
        ##for block in llm_input[-1].blocks[::-1]:
        ##    if isinstance(block, TextBlock):
        ##        block.text = self.state_prompt.format(state=state, msg=user_msg_str)
        ##        break
        await ctx.store.set("formatted_input_with_state", True)
        return AgentSetup(
            input=llm_input,
